//after refacturization met issues, version below is working the same as in model
...
generated quantities {
    ...
    real<lower=0.0> potential_rich_mulog_coeff = normal_rng(0.05, 0.001);
    real<lower=0.0> potential_rich_sigmalog_coeff = normal_rng(0.1, 0.01);
    ...
    real<lower=0.0> sigma = normal_rng(0.1, 0.01);

    real mulog = (1-potential_rich)*log(pow(mu,2)/sqrt(pow(mu,2)+pow(sigma,2))) + potential_rich*0.05;
    real sigmalog =(1-potential_rich)*normal_rng(1, 0.1) + potential_rich*0.1;
